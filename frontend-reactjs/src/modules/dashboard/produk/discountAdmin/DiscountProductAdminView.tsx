...
export default function DiscountProductAdminView() {
    ...
    if (hasSearchFilter) {
      filteredCategories = filteredCategories.filter(
        (category) =>
          ...
          category.user.email.toLowerCase().includes(filterValue.toLowerCase())
      );
    }
    if (
      statusFilter !== "all" &&
  ...
  const renderCell = React.useCallback(
    (discount, columnKey) => {
      const cellValue =
        columnKey === "user.email" ? discount.user.email : discount[columnKey];
