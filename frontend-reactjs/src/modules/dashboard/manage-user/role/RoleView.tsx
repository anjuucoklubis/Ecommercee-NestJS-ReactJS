...
import DetailRoleView from "./DetailRoleView.tsx";
...
export default function RoleView() {
  // ========================
  const { roles, columns } = RoleViewModelGet();
  const { formatDate } = DateComponenttt();
  const { isOpen, onOpen, onClose } = useDisclosure();

  const [isOpenDetailRole, setIsOpenDetailRole] = useState(false);
  const [roleIdToDetail, setRoleIdToDetail] = useState(null);
  const handleView = (id) => {
    setRoleIdToDetail(id);
    setIsOpenDetailRole(true);
  };

  const [isOpenUpdateRole, setIsOpenUpdateRole] = useState(false);
  const [roleIdToUpdate, setRoleIdToUpdate] = useState(null);
  const handleEdit = (id) => {
    setRoleIdToUpdate(id);
    setIsOpenUpdateRole(true);
  };
  const closeModal = () => {
    setIsOpenUpdateRole(false);
    setRoleIdToUpdate(null);
    setIsOpenDetailRole(false);
  };
  ...
  const filteredItems = React.useMemo(() => {
    ...
    if (hasSearchFilter) {
      filteredUsers = filteredUsers.filter((roles) =>
        roles.role_name.toLowerCase().includes(filterValue.toLowerCase())
      );
    }
    if (
      statusFilter !== "all" &&
      ...
      filteredUsers = filteredUsers.filter((roles) =>
        Array.from(statusFilter).includes(roles.role_name)
      );
    }
    ...
    (role, columnKey) => {
      ...
      switch (columnKey) {
        case "name":
          return (
            <User
              avatarProps={{ radius: "lg", src: role.avatar }}
  ...
  return (
    <PartialView>
      ...
      <DetailRoleView
        isOpenDetailRole={isOpenDetailRole}
        onClose={closeModal}
        roleId={roleIdToDetail || ""}
      />
      <UpdateRoleView
      ...
      />
      <AddRoleView isOpen={isOpen} onClose={onClose} />
